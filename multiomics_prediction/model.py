...
class MultiOmicsModel_2Mb(nn.Module):
    def __init__(self, num_tasks: int = 4, body_dim: int = 384, pretrained_path: str = None):
        super().__init__()
        ...
        self.pos_encoder = PositionalEncoding1D(body_dim, max_len=10000)
        encoder_layer = nn.TransformerEncoderLayer(d_model=body_dim, nhead=8, dim_feedforward=body_dim * 4, dropout=0.2, activation='gelu', batch_first=True)
        ...
        self.final_head = nn.Sequential(nn.Conv1d(64, num_tasks, kernel_size=1), nn.Softplus())
        if pretrained_path:
...
class DNAEncoderNew(nn.Module):
    ...
    Matches strategy used in dna2hic_10kb.
    Input: (B, 4, 2,240,000)
...
class MultiOmicsPredictor(nn.Module):
    def __init__(
        ...
        self.track_decoders = nn.ModuleDict({
    ...
    def forward(self, dna_seq: torch.Tensor, hic_input: torch.Tensor) -> torch.Tensor:
        ...
        for track in self.track_names:
            out = self.track_decoders[track](x)                                      
            outputs.append(out)
        final_out = torch.cat(outputs, dim=1)                                                   
        return final_out
if __name__ == "__main__":
    track_names = ["ATAC", "CTCF", "H3K27ac", "H3K4me3"]
    model = MultiOmicsPredictor(track_names=track_names, pretrained_path=None)
    dna = torch.randn(1, 4, 2240000)
    hic = torch.randn(1, 1, 224, 224)
    out = model(dna, hic)
    print("Output shape:", out.shape)
    assert out.shape == (1, 4, 2240)
    print("Success.")
